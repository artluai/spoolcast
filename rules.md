...
- **Shot-list field coherence: image_source, image_path, and beat_description move together.** When a chunk's `image_source` changes (e.g., `broll_image` → `generated`, or `generated` → `broll_image`), `image_path` MUST be updated in the same edit to point at the correct asset for the new source kind, AND `beat_description` must match. A chunk with `image_source: generated` but `image_path` pointing at a fetched-asset meme JPG will quietly serve the old meme to the renderer — the new beat_description and regenerated scene PNG are silently ignored because downstream scripts read `image_path`, not `image_source`. Also remove any source-kind-specific fields that no longer apply (e.g., `broll_source_kind`, `broll_framing` when converting away from broll). Caught on dev-log-02 mobile export: C18/C24/C39 were regenerated as illustrations (new beat_descriptions written, scene PNGs rendered at `source/generated-assets/scenes/CXX.png`), but their shot-list entries still had `image_source: broll_image` and `image_path: source/fetched-assets/memes/meme-XXX.jpg`. Smart-crop read `image_path`, cropped the memes, and Part 2 opened with the Picard facepalm meme viewers thought had been replaced. The beat_description can't fix this — downstream pipelines trust `image_path`.

- **Text-card → visible-action conversion: recalibrate `hold_duration_sec`.** When a chunk converts from a text-card beat to a visible-action beat (per the text-card density cap), the chunk's `hold_duration_sec` must be reset — not inherited from the pre-conversion value. Text-card holds are sized for the viewer's *reading time* (`word_count × 0.35s` read-time floor); visible-action holds are sized for *visual registration* (how long the viewer needs to parse the scene), which is typically ≤1s once the narration has played. Carrying the text-card hold forward after conversion turns the new scene into dead air: the scene lands in ~1s, then the hold keeps the frame frozen for another 2–4s with no content to carry it. Caught on dev-log-02 v5: `Capture it / Replay it / Don't let it remember` held 2/3/4s silently after conversion to jar-scene / jar-opening / cloud-block beats. Rule: on conversion, default `hold_duration_sec` to 0.3–1.0s (scene registration) unless a specific end-of-build landing or declared ending archetype earns more, and audit the silence budget against the new value.
...
- **Review artifacts are exactly two things: the short version in chat, and the final shot-list xlsx.** Source analysis, screenplay v1/v2/v3 prose, voiceover scripts — written to disk for traceability but NEVER linked to the user for review. See STORY.md § Review-Artifact Policy. **After every screenplay version is drafted to disk, AND after any visual-intent patch (broll/meme flagging, asset reassignments, SFX-landing flags), surface the short bulleted version in chat as the review artifact. Don't wait for the user to ask.**
...
- **Don't invent timeframes — look them up.** Any "a few weeks ago" / "last month" / "earlier this year" in narration must come from a real source: tracker, git log, file timestamps, publish date. If the source doesn't say, rewrite the line to not depend on time.
- **Script-level inflection cues (ellipsis, em-dashes, sentence fragments) carry the narration's emotional shape — try them before reaching for production-side fixes.** When a line falls flat on the silent read, rewrite the punctuation/structure first; only after that try TTS prosody, overlays, SFX, or chunk-level pacing tricks. See STORY.md § Heuristics #5b.
